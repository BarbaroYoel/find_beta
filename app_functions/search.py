    ...
    for _, row in df.iterrows():
        ...
        if day in hours:
          ...
          if schedule and "_" in schedule:
            opening_time, closing_time = schedule.split("_")
...
def search_by_services(df, services):
    ...
    for _, row in df.iterrows():
       ...
       for servicio in services:
            if servicio not in services or services_dic[servicio] is None or not services_dic[servicio]:
                cumple_todos = False
                break
...
def search_by_dish_and_price(df, dish, dish_max_price, threshold=80):
    ...
    df_filtered = pd.DataFrame(list(filtered_rows), columns=df.columns)  
    if df_filtered.empty:
